        ...
        if run_mv:
            try:
                target_mv = optimize_portfolio(
                    ...
                    target_net_exposure=float(cfg["target_net_exposure_mv"]),
                    risk_aversion=float(cfg["mv_risk_aversion"]),
        ...
        if run_cvar:
            ...
            try:
                target_cvar = optimize_portfolio_cvar(
                    ...
                    max_position=0.10, min_position=0.0,
                    target_net_exposure=float(cfg["target_net_exposure_cvar"]),
                    risk_aversion=float(cfg["cvar_risk_aversion"]),
                    turnover_penalty=float(cfg["cvar_turnover_penalty"]),
        ...
        if run_robust:
            try:
                target_rob = optimize_portfolio_robust(
                    ...
                    n_simulations=int(cfg["michaud_n_simulations"]),
                    max_position=0.10, min_position=0.0,
                    target_net_exposure=float(cfg["target_net_exposure_robust"]),
                    risk_aversion=float(cfg["robust_risk_aversion"]),
